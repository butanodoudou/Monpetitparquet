...
export default function TeamPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      fetch(`/api/leagues/${leagueId}/lineup`, { headers: { Authorization: `Bearer ${token}` } }).then(r => r.ok ? r.json() : null),
    ]).then(([teamData, standingsData, wd, lineupData]) => {
      setPlayers(teamData.players ?? []);
      setTotalScore(teamData.totalScore ?? 0);
      setStandings(standingsData ?? []);
      if (wd) setWeekData(wd);
      if (lineupData?.starterIds) setStarterIds(new Set(lineupData.starterIds));
        ...
        <div className="px-4 pt-4">
          <div className="card bg-gradient-to-br from-slate-800 to-slate-900 text-center py-5">
              ...
              <div className="mt-4 flex items-stretch justify-center gap-3">
                <div className="bg-slate-700/60 rounded-xl px-4 py-2.5 text-center">
                  <div className="text-[10px] text-slate-500 uppercase tracking-wide mb-0.5">
                    J{weekData.currentWeek} (titulaires)
                  </div>
            ...
            <Link href={`/leagues/${leagueId}/lineup`}
              className="mt-4 inline-flex items-center gap-1.5 bg-slate-700 hover:bg-slate-600 text-slate-300 text-xs font-semibold px-4 py-2 rounded-full transition-colors">
            ...
            players.length === 0
              ...
              : <div className="space-y-3">
                  {players.map(p => {
                    ...
                    return (
                      <div key={p.id} className={`card ${isStarter ? 'border-brand/30' : 'opacity-80'}`}>
                        <div className="flex items-center gap-2 mb-2">
                          <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                            isStarter ? 'bg-brand/20 text-brand' : 'bg-slate-700 text-slate-500'
                          }`}>
                        ...
                        <div className="flex gap-4 mt-2 pt-2 border-t border-slate-700 text-xs text-slate-400">
                          ...
                          <span className="ml-auto">
                            {weekData ? (
                              <>J{weekData.currentWeek}: <span className={`font-bold ${isStarter ? 'text-brand' : 'text-slate-500'}`}>{p.last_week_fantasy.toFixed(1)}</span></>
                            ) : (
