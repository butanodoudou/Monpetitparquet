              ...
              <>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {TIER_ORDER.map(tier => {
                    ...
                    return (
                      <motion.button key={tier} whileTap={{ scale: 0.95 }} disabled={!canAfford || loading} onClick={() => openPack(tier)}
                        className={`relative p-4 rounded-2xl border-2 text-left transition-all ${
                          canAfford ? `bg-gradient-to-br ${cfg.bg} border-slate-600` : 'bg-slate-800/30 border-slate-700/30 opacity-40'
                        }`}>
                        ...
                        <div className={`absolute top-3 right-3 font-black text-xs ${canAfford ? cfg.color : 'text-slate-600'}`}>{PACK_PRICES[tier]}cr</div>
                      </motion.button>
                    );
                  })}
                </div>
                <div className="mt-4 text-center text-xs text-slate-600">Chaque pack révèle 3 joueurs · Tu en gardes 1</div>
              </>
            )}
          </motion.div>
        )}
          ...
          <div className="w-full max-w-sm">
            ...
            <div className="flex gap-3 justify-center">
              {packPlayers.map((player, i) => {
                ...
                return (
                  <motion.div key={player.id} style={{ perspective: 1000 }} className="flex-1"
                    animate={isChosen ? { scale: 1.08 } : isRejected ? { scale: 0.88, opacity: 0.3 } : { scale: 1, opacity: 1 }}
                    transition={{ duration: 0.4 }}>
                    <motion.div style={{ transformStyle: 'preserve-3d', position: 'relative', height: 180 }}
                      animate={{ rotateY: revealed[i] ? 180 : 0 }} transition={{ duration: 0.55, ease: 'easeInOut' }}>
                      <div style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden' }}
                        className="absolute inset-0 rounded-2xl bg-gradient-to-br from-slate-700 to-slate-800 border-2 border-slate-600 flex items-center justify-center">
                        <motion.div animate={{ opacity: [0.3, 0.8, 0.3] }} transition={{ duration: 1.5, repeat: Infinity }} className="text-4xl">🏀</motion.div>
                      </div>
                      <div style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}
                        className={`absolute inset-0 rounded-2xl bg-gradient-to-br ${cfg.bg} border-2 flex flex-col p-2 cursor-pointer ${
                          phase === 'picking' && !chosenId ? 'border-slate-500' : 'border-slate-700'
                        ...
                        <div className="flex-1 flex items-center justify-center">
                          {player.photo_url
                            ...
                            : <div className="w-12 h-12 rounded-full bg-slate-700 flex items-center justify-center text-slate-400 text-xs font-bold">#{player.jersey_number ?? '?'}</div>
                          }
                        </div>
                        <div className="text-center mt-1">
                          <div className="text-slate-100 font-bold text-[11px] truncate">{player.name.split(' ').pop()}</div>
