...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const [{ data: league }, { data: membership }] = await Promise.all([
    supabase.from('leagues').select('commissioner_id, draft_status').eq('id', leagueId).single(),
  ...
  const membersData = (members ?? []).map(m => ({
    ...
    credits: m.draft_credits ?? 500_000,
    hasSubmitted: false,
  }));
  ...
  if (!currentPack) {
    return NextResponse.json({
      ...
      myCredits: membership.draft_credits ?? 500_000,
      myRemainingSlots,
      members: membersData,
      currentPack: null,
